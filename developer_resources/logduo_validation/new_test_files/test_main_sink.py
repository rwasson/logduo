...
def test_20_initialize_missing_main_dir_raises(tmp_path):
    ...
    original_main_dir = log._runtime.main_sink_log_dir_path_abs
    try:
        ...
        with pytest.raises(
            RuntimeError,
            match="main_sink_log_dir_path_abs missing",
        ):

            _initialize_main_sink_log(log)

    finally:
        log._runtime.main_sink_log_dir_path_abs = original_main_dir
        log.close()
...
def test_21_initialize_missing_log_path_raises(tmp_path):
    ...
    log = Duo()
    log.configure(log_dir_path=tmp_path)
    original_log_path = log._runtime.main_sink_log_file_path_abs
    ...
    try:
        ...
        with pytest.raises(
            RuntimeError,
            match="main_sink_log_file_path_abs missing",
        ):
            _initialize_main_sink_log(log)

    finally:
        log._runtime.main_sink_log_file_path_abs = original_log_path
        log.close()


# --- test_22_main_log_writes_trace_at_verbosity_three() ----------------------
def test_22_main_log_writes_trace_at_verbosity_three(tmp_path):
    ...
    log.trace("main sink trace message")
    log.close()

    content = _read_file(_find_main_log(tmp_path))

    assert "TRACE" in content
    assert "main sink trace message" in content
