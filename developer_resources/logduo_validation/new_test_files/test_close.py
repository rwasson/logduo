...
# --- test_19_close_completes_queued_loguru_writes() -----------------------------
def test_19_close_completes_queued_loguru_writes(tmp_path):
    log = Duo()
    ...
    for index in range(100):
        log.trace(f"queued trace message {index}")

    log.close()
    content = log_path.read_text(encoding="utf-8")

    assert "queued trace message 0" in content
    assert "queued trace message 99" in content
    assert "logging ended" in content
