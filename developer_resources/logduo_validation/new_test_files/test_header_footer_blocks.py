...
def test_13_main_log_footer_wraps_created_files(tmp_path: Path):
    ...
    footer_heading = "Logduo-managed files created this run:"
    assert footer_heading in log_content

    created_files_section = log_content.split(
        footer_heading,
        maxsplit=1,
    )[1]
    ...
    print(repr(created_files_section))

    # Rejoin text that may have been split across wrapped lines.
    compact_section = "".join(
        line.strip()
        for line in created_files_section.splitlines()
    )

    assert "session.log" in compact_section
    assert "config_table.txt" in compact_section

    # Confirm that wrapping actually occurred.
    nonblank_lines = [
        line
        for line in created_files_section.splitlines()
        if line.strip()
