"""
test_pytest_harness_import_error.py
...
import developer_resources.pytest_harness.record_buder as module
from developer_resources.pytest_harness.constants_and_classes import TestFileStatus
...
class _FakeLog:
    def __init__(self) -> None:
        self.join_calls = 0
        self.new_logger_calls: list[tuple[tuple[Any, ...], dict[str, Any]]] = []
        self.logged_lines: list[str] = []

    def join(self) -> None:
        self.join_calls += 1

    def new_logger(self, *args: Any, **kwargs: Any):
...
def test_02_supports_error_plural_key_and_error_outcome(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    source_dir = tmp_path / "src"
...
def test_05_unexpected_outcome_raises_and_removes_temporary_report(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    source_dir = tmp_path / "src"
    ...
    with pytest.raises(RuntimeError, match="Unexpected pytest outcome"):
        module._build_test_file_record(
            test_file_path=test_file,
...
def _install_fake_runtime(
    ...
    *,
    ...
    def fake_popen(cmd, **kwargs):
        ...
        report_path = Path(report_arg.split("=", maxsplit=1)[1])
    ...
    monkeypatch.setattr(
        ...
        lambda text: text.replace("\x1b[31m", "").replace("\x1b[0m", ""),
    )
