...
from developer_resources.pytest_toolkit.test_utils import (
        _find_main_log, _read_file, _find_new_logger_log,
        _find_file,
    )
from logduo.internals.engine.runtime_classes import RuntimeRecord
from logduo.internals.formatters.console_header_footer_builders import _build_console_header, \
    _build_console_footer, _render_rich_label_value_row
from logduo.internals.formatters.header_footer_formatters import _build_auto_header_info_rows, \
    _build_auto_footer_info_rows, _build_shortened_file_path_display_label, _derive_label_pad, \
...
#  --- helper for tests' CreatedFileRecord -------------------------------------
def _make_cfr(
    path: Path,
    *,
...
def test_10_script_mode_populates_script_path(tmp_path: Path):
    ...
    script_path = Path(
        __file__).parent.parent / "test_files" / "test_helper_files" / "script_simple.py"
    print(" ")
    print("***********************************************************")
